...
bool32 ObjectEventInteractionWaterBerryTree(void)
{
    ...
    case BERRY_STAGE_FLOWERING:
        // Water all stages so the berry stays watered as it grows
        tree->watered1 = TRUE;
        tree->watered2 = TRUE;
        tree->watered3 = TRUE;
        tree->watered4 = TRUE;
        tree->minutesUntilNextStage /= 2;
        break;
    default:
        return FALSE;
    }
...
void TryReseedBerryTrees(void)
{
    ...
    if (currentSteps - sLastReseedSteps >= 1530)
    {
        int i;
        struct BerryTree *tree;
        ...
        for (i = 0; i < BERRY_TREES_COUNT; i++)
        {
            ...
            {
                if ((Random() % 100) < 10) // 25% chance to reseed
                {
                    tree->stage = BERRY_STAGE_PLANTED;
                    tree->minutesUntilNextStage = GetStageDurationByBerryType(tree->berry);
                    tree->berryYield = 0;
