...
static double GetPkmnExpMultiplier(u8 level)
{
    ...
    // Clamp to allowed range, but ensure no penalties
    if (avgDiff >= 0)
        avgDiff = 0;  // No bonus or penalty for overleveled/at-party-level mons
    else if (avgDiff <= -14)
        avgDiff = -14;  // Cap the bonus for underleveled mons

    avgDiff += 14; // map [-14..0] to [0..14]
...
static void Cmd_critcalc(void)
{
    ...
    if ((gBattleMons[gBattlerTarget].ability != ABILITY_BATTLE_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR)
     && !gSideTimers[GET_BATTLER_SIDE(gBattlerTarget)].mistTimer
    ...
    case 4:
        if (!gPaletteFade.active && gMain.callback2 == BattleMainCB2)
        {
            ...
            else
            {
                ...
                /* HM protection removed: allow forgetting/replacing any move, including HMs */
                gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
...
void CalculateCatchResult(void)
{
    ...
    if (gLastUsedItem > ITEM_SAFARI_BALL)
    {
        ...
        case ITEM_PREMIER_BALL:
            {
                u8 bonus = CatchMinigame_GetBonus();
                u8 tierMultiplier;
                u8 bonusMultiplier;
                
                // Set tier multiplier based on buttons pressed
                if (bonus == 0)
