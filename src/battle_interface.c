...
#define LAST_BALL_WIN_Y       ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) ? 76 : 66)
#define LAST_USED_BALL_Y      (LAST_BALL_WIN_Y + 4)  // Ball in lower portion of window
#define LAST_BALL_WIN_X_F     (LAST_USED_BALL_X_F)
#define LAST_BALL_WIN_X_0     (LAST_USED_BALL_X_0)
#define LAST_BALL_RBTN_Y      (LAST_BALL_WIN_Y - 10) // R button sits above, intersecting top of window

#define TAG_LAST_USED_BALL     0xD750
#define TAG_LAST_BALL_WINDOW   0xD751
#define TAG_LAST_BALL_RBUTTON  0xD752

// R button graphic from union_room_chat (16x16, compressed)
static const u32 sRButtonGfx[] = INCBIN_U32("graphics/union_room_chat/r_button.4bpp.lz");
static const u16 sRButtonPal[] = INCBIN_U16("graphics/union_room_chat/interface.gbapal");

static const struct CompressedSpriteSheet sSpriteSheet_RButton = {
    sRButtonGfx, 0x0080, TAG_LAST_BALL_RBUTTON
};

static const struct SpritePalette sSpritePalette_RButton = {
    sRButtonPal, TAG_LAST_BALL_RBUTTON
};

// 32x32 window graphic (4bpp, 512 bytes = 16 tiles)
// Simple dark underlay with rounded corners - no text
static const u8 ALIGNED(4) sLastUsedBallWindowGfx[] = {
    // === Row 1 (tiles 0-3): Top edge ===
...
// Palette for window underlay
static const u16 ALIGNED(4) sLastUsedBallWindowPal[] = {
    RGB(0, 0, 0),      // 0: transparent
    RGB(4, 4, 6),      // 1: dark outline
    RGB(10, 10, 14),   // 2: semi-dark fill
    RGB(31, 31, 31),   // 3: unused
...
static void SpriteCB_LastUsedBallRButton(struct Sprite *sprite);
...
// R button sprite - uses existing graphics from union_room_chat
static const u32 sLastUsedBallRButtonGfx[] = INCBIN_U32("graphics/union_room_chat/r_button.4bpp.lz");
static const u16 sLastUsedBallRButtonPal[] = INCBIN_U16("graphics/union_room_chat/interface.gbapal");

static const struct CompressedSpriteSheet sSpriteSheet_LastUsedBallRButton = {
    .data = sLastUsedBallRButtonGfx, .size = 0x0080, .tag = TAG_LAST_BALL_RBUTTON
};

static const struct SpritePalette sSpritePalette_LastUsedBallRButton = {
    .data = sLastUsedBallRButtonPal, .tag = TAG_LAST_BALL_RBUTTON
};

static const struct OamData sOamData_LastUsedBallRButton = {
    .y = 0,
    .affineMode = ST_OAM_AFFINE_OFF,
    .objMode = ST_OAM_OBJ_NORMAL,
    .mosaic = FALSE,
...
static const struct SpriteTemplate sSpriteTemplate_LastUsedBallRButton = {
    .tileTag = TAG_LAST_BALL_RBUTTON,
    .paletteTag = TAG_LAST_BALL_RBUTTON,
    .oam = &sOamData_LastUsedBallRButton,
    .anims = gDummySpriteAnimTable,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCB_LastUsedBallRButton
};
...
// Sprite callback for R button sprite - handles slide-in animation
static void SpriteCB_LastUsedBallRButton(struct Sprite *sprite)
{
    // Slide in from left, positioned to intersect top of window
    if (sprite->x < LAST_BALL_WIN_X_F)
        sprite->x += 2;
}
...
void TryAddLastUsedBallItemSprites(void)
{
    ...
    if (gBattleStruct->ballSpriteIds[1] == 0xFF)
    {
        ...
        spriteId = CreateSprite(&sSpriteTemplate_LastUsedBallWindow, LAST_BALL_WIN_X_0, LAST_BALL_WIN_Y, 5);
    ...
    // Create R button sprite (positioned to intersect top of window)
    if (gBattleStruct->ballSpriteIds[2] == 0xFF)
    {
        LoadCompressedSpriteSheet(&sSpriteSheet_LastUsedBallRButton);
        LoadSpritePalette(&sSpritePalette_LastUsedBallRButton);
        spriteId = CreateSprite(&sSpriteTemplate_LastUsedBallRButton, LAST_BALL_WIN_X_0 - 1, LAST_BALL_WIN_Y - 14, 4);
        if (spriteId != MAX_SPRITES)
        {
            gBattleStruct->ballSpriteIds[2] = spriteId;
        }
    }
...
void TryHideLastUsedBallSprites(void)
{
    ...
    // Destroy R button sprite
    if (gBattleStruct->ballSpriteIds[2] != 0xFF && gBattleStruct->ballSpriteIds[2] < MAX_SPRITES)
    {
        FreeSpriteTilesByTag(TAG_LAST_BALL_RBUTTON);
        FreeSpritePaletteByTag(TAG_LAST_BALL_RBUTTON);
        DestroySprite(&gSprites[gBattleStruct->ballSpriteIds[2]]);
