...
static bool8 TryStartStepCountScript(u16 metatileBehavior)
{
    ...
    if (!(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_FORCED_MOVE) && !MetatileBehavior_IsForcedMovementTile(metatileBehavior))
    {
        ...
        {
            ScriptContext_SetupScript(RustboroCity_DevonCorp_3F_EventScript_MrStoneCall);
            return TRUE;
        }
...
{
    // No encounters while at max speed on Mach Bike
    if ((gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_MACH_BIKE) && GetPlayerSpeed() == PLAYER_SPEED_FASTEST)
        return FALSE;
...
static void SetupWarp(struct MapHeader *unused, s8 warpEventId, struct MapPosition *position)
{
    ...
    if (warpEvent->mapNum == MAP_NUM(MAP_DYNAMIC)
     && warpEvent->mapGroup == MAP_GROUP(MAP_DYNAMIC))
    {
    ...
    else
    {
        ...
        if (mapHeader->events->warps[warpEvent->warpId].mapNum == MAP_NUM(MAP_DYNAMIC)
         && mapHeader->events->warps[warpEvent->warpId].mapGroup == MAP_GROUP(MAP_DYNAMIC))
