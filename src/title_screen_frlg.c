...
static const u8 sBorderBgTiles[] = INCBIN_U8("graphics/title_screen/frlg/border_bg.4bpp.lz");
static const u8 sBorderBgMap[] = INCBIN_U8("graphics/title_screen/frlg/border_bg.bin.lz");
static const u32 sSlash_Gfx[] = INCBIN_U32("graphics/title_screen/frlg/slash.4bpp.lz");
static const u16 sFlames_Pal[] = INCBIN_U16("graphics/title_screen/frlg/flames.gbapal");
static const u32 sFlames_Gfx[] = INCBIN_U32("graphics/title_screen/frlg/flames.4bpp.lz");
static const u32 sBlankFlames_Gfx[] = INCBIN_U32("graphics/title_screen/frlg/blank_flames.4bpp.lz");
static const u16 sLeaves_Pal[] = INCBIN_U16("graphics/title_screen/frlg_lg/leaves.gbapal");
static const u32 sLeaves_Gfx[] = INCBIN_U32("graphics/title_screen/frlg_lg/leaves.4bpp.lz");
static const u32 sStreak_Gfx[] = INCBIN_U32("graphics/title_screen/frlg_lg/streak.4bpp.lz");
...
static const union AnimCmd *const sSpriteAnim_LeafTable[] = {
    sSpriteAnim_Leaf
};
...
static const struct SpriteTemplate sSpriteTemplate_Leaf = {
    .tileTag = TILE_TAG_FLAME_OR_LEAF,
    .paletteTag = PAL_TAG_DEFAULT,
    .oam = &sOamData_FlameOrLeaf,
    .anims = sSpriteAnim_LeafTable,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy
};
...
void CB2_InitTitleScreen_Frlg(void)
{
    switch (gMain.state)
    {
    default:
    ...
    case 0:
        ...
        break;
    case 1:
        if (gIntroPairIndex == 2)
        {
            LoadPalette(gGraphics_TitleScreen_LG_GameTitleLogoPals, BG_PLTT_ID(0), 13 * PLTT_SIZE_4BPP);
            DecompressAndCopyTileDataToVram(0, gGraphics_TitleScreen_LG_GameTitleLogoTiles, 0, 0, 0);
            DecompressAndCopyTileDataToVram(0, gGraphics_TitleScreen_LG_GameTitleLogoMap, 0, 0, 1);
            LoadPalette(gGraphics_TitleScreen_LG_BoxArtMonPals, BG_PLTT_ID(13), PLTT_SIZE_4BPP);
            DecompressAndCopyTileDataToVram(1, gGraphics_TitleScreen_LG_BoxArtMonTiles, 0, 0, 0);
            DecompressAndCopyTileDataToVram(1, gGraphics_TitleScreen_LG_BoxArtMonMap, 0, 0, 1);
            LoadPalette(gGraphics_TitleScreen_LG_BackgroundPals, BG_PLTT_ID(15), PLTT_SIZE_4BPP);
            ...
            LoadPalette(gGraphics_TitleScreen_LG_BackgroundPals, BG_PLTT_ID(14), PLTT_SIZE_4BPP);
        }
        else
        {
...
static void SetTitleScreenScene_FadeIn(s16 *data)
{
    ...
    case 8:
        if (!IsBlendPalettesGraduallyTaskActive(0))
        {
            ...
            ShowBg(0);
            CpuCopy16(gIntroPairIndex == 2 ? gGraphics_TitleScreen_LG_BoxArtMonPals : gGraphics_TitleScreen_BoxArtMonPals, &gPlttBufferUnfaded[BG_PLTT_ID(13)], PLTT_SIZE_4BPP);
...
static void SetTitleScreenScene_Restart(s16 *data)
{
    ...
    case 1:
        ...
        break;
    case 2:
        if ((gMPlayInfo_BGM.status & 0xFFFF) == 0 && !gPaletteFade.active)
        {
...
static void SetTitleScreenScene_Cry(s16 *data)
{
    ...
    case 0:
        if (!gPaletteFade.active)
        {
            PlayCry_Normal(gIntroPairIndex == 2 ? SPECIES_VENUSAUR : TITLE_SPECIES, 0);
            DeactivateSlashSprite(tSlashSpriteId);
            data[2] = 0;
            tState++;
        }
...
static void LoadSpriteGfxAndPals(void)
{
    ...
    if (gIntroPairIndex == 2)
    {
        for (i = 0; i < (s32)ARRAY_COUNT(sSpriteSheets_LG); i++)
            LoadCompressedSpriteSheet(&sSpriteSheets_LG[i]);
        LoadSpritePalettes(sSpritePals_LG);
    }
    else
    {
        for (i = 0; i < (s32)ARRAY_COUNT(sSpriteSheets); i++)
...
{
    u8 spriteId = CreateSprite(&sSpriteTemplate_Leaf, DISPLAY_WIDTH, y, 0);
    if (spriteId != MAX_SPRITES)
    {
...
static void SpriteCallback_Streak(struct Sprite *sprite)
{
    ...
    if (sprite->x < -16)
    {
        ...
        sprite->data[7]++;
        if (sprite->data[7] >= (s16)ARRAY_COUNT(sStreakYPositions))
            sprite->data[7] = 0;
...
static void Task_LeafSpawner(u8 taskId)
{
    ...
    case 1:
        ...
        {
            tTimer = 0;
            tDelay = (TitleScreen_rand(taskId, 3) % 6) + 6;
            ...
            yspeed = (TitleScreen_rand(taskId, 3) % 4) - 2;
            y = (TitleScreen_rand(taskId, 3) % 88) + 32;
