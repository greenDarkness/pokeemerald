...
static void CreateHatchedMon(struct Pokemon *egg, struct Pokemon *temp)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        pp[i] = GetMonData(egg, MON_DATA_PP1 + i);
    }
    ppBonuses = GetMonData(egg, MON_DATA_PP_BONUSES);
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        SetMonData(temp, MON_DATA_PP1 + i, &pp[i]);
    }
...
static u8 EggHatchCreateMonSprite(u8 useAlt, u8 state, u8 partyId, u16 *speciesLoc)
{
    ...
    if (useAlt == FALSE)
    {
        mon = (partyId == PARTY_SIZE) ? &gEggSlot : &gPlayerParty[partyId];
...
static void EggHatchSetMonNickname(void)
{
    ...
    if (gSpecialVar_0x8004 == PARTY_SIZE)
    {
        ...
        if (gStringVar3[0] != EOS)
            SetMonData(&gEggSlot, MON_DATA_NICKNAME, gStringVar3);
...
static void CB2_EggHatch(void)
{
    ...
    case 3:
        ...
        if (gSprites[sEggHatchData->eggSpriteId].callback == SpriteCallbackDummy)
        {
            if (sEggHatchData->eggPartyId == PARTY_SIZE)
                species = GetMonData(&gEggSlot, MON_DATA_SPECIES);
            else
    ...
    case 5:
        ...
        if (sEggHatchData->eggPartyId == PARTY_SIZE)
            GetMonNickname2(&gEggSlot, gStringVar1);
        else
    ...
    case 10:
        ...
        case 0: // Yes
            if (sEggHatchData->eggPartyId == PARTY_SIZE)
            {
                GetMonNickname2(&gEggSlot, gStringVar3);
                species = GetMonData(&gEggSlot, MON_DATA_SPECIES);
                gender = GetMonGender(&gEggSlot);
                personality = GetMonData(&gEggSlot, MON_DATA_PERSONALITY, 0);
            }
            else
            {
